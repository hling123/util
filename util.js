var util = {
    ...
    _isDocument: function (value) {
        return value != null && !!value.nodeType && value.nodeType == value.DOCUMENT_NODE
    },
    ...
    _isPlainObject: function (value) {
        return this._isObject(value) && !this._isWindow(value) && Object.getPrototypeOf(value) == Object.prototype
    },
    ...
    _parseTime: function (time, cFormat) {
        ...
        if (typeof time === "object") { // new Date()
            ...
            if (typeof time === "string" && /^[0-9]+$/.test(time)) { //时间戳字符串
                time = parseInt(time);
            }
            if (typeof time === "number" && time.toString().length === 10) {// 10位时间戳
                time = time * 1000;
            }
